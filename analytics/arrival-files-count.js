function formArrivalFileCountOracle(input) {
  ...
  if (isSfrAccount && input?.filter?.fileIdInfo) {
    ...
    if(input.search.includes('-')) {
      ...
      // escaped_search_text = escaped_search_text.replace(/'/g, "''");
      subQuery += ` AND (CONTAINS(STATE, '%"${input.search}"%', 1) > 0) `;
    }
    else{
    subQuery += ` AND (CONTAINS(STATE, '%${escaped_search_text}%', 1) > 0) `;
    }
  ...
  // view query
  let query = `SELECT COUNT(*) AS record_count From(WITH subquery AS(${subQuery}) SELECT EVENTID FROM ${input.view} LEFT JOIN subquery ON ${input.view}.coreid = subquery.SUBCOREID WHERE eventtimestamp IS NOT NULL `
...
// module.exports = formArrivalFileCountOracle;

let input = {"libraryScriptId":"667e9099618609e5f39e6cdc","view":"xfbtransfer_h","location":"34235236163","limit":"9","offset":"0","search":"for logs.txt","transferDuration":"Recent","from":null,"to":null,"orderBy":null,"order":null,"filter":null,"isLegacy":false,"application":"NYL-AdvancedRoute","partnerAccounts":null}

formArrivalFileCountOracle(input)