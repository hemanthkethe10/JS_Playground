function formDownloadFileOracle(input){
    let application = input.application || "NYL-AvancedRoute";
  ...
  if (input?.filter?.businessUnit && input?.filter?.businessUnit.toLowerCase() != "all") {
  ...
  if (input?.filter?.client && input?.filter?.client.toLowerCase() != "all") {
    subQuery += ` AND UPPER(ClientName) = UPPER('${input.filter.client}')`
  }
  if (input?.filter?.partnerName && input?.filter?.partnerName.toLowerCase() != "all") {
  ...
  if (input?.filter?.partnerAccount && input?.filter?.partnerAccount.toLowerCase() != "all") {
    subQuery += ` AND UPPER(SourceAccount) = UPPER('${input.filter.partnerAccount}')`
  }
  if (input?.filter?.fileId && input?.filter?.fileId.toLowerCase() != "all") {
    subQuery += ` AND UPPER(fileId) = UPPER('${input.filter.fileId}')`;
  }
  if (input?.filter?.fileIdInfo) {
    ...
    if (input?.filter?.fileIdInfo.fileNameingPattern.enabled) {
    ...
    if (input?.filter?.fileIdInfo.idValue.enabled) {
      subQuery += ` AND UPPER(IDVALUE) = UPPER('${input.filter.fileIdInfo.idValue.value}')`;
    }
    if (input?.filter?.fileIdInfo.division.enabled) {
    ...
    // view query
    let query = `WITH subquery AS(${subQuery}) SELECT TO_CHAR( TO_DATE('1970-01-01', 'YYYY-MM-DD') +(eventtimestamp / 1000 / 86400), 'MM-DD-YYYY HH24:MI:SS') AS "DateTime", EVENTID as "eventId", cycleid AS "CycleId", parentcycleid AS "ParentCycleId", ${input.view}.coreid AS "CoreId", CASE WHEN subquery.has_failed = 1 AND subquery.count_valid_states = 0 THEN 'Failure' WHEN subquery.has_failed = 1 AND subquery.count_valid_states > 0 THEN 'Partial Failure' ELSE 'Successful' END AS "Status", ProtocolFileName AS "ProtocolFileName", FileSize AS "FileSize", UserId AS "UserId", Protocol AS "Protocol", CASE WHEN IsServer = 0 THEN 'PUSH' WHEN IsServer = 1 AND site != 'N2' THEN 'PULL' WHEN IsServer = 1 AND site = 'N2' THEN 'PUSH' END AS "ArrivalMethod", CASE UserParameter1 WHEN 'E' THEN 'External' WHEN 'I' THEN 'Internal' ELSE 'Not Specified' END AS "SourceType", GroupName AS "GroupName", ISALERT, ISEND, ISEXCEPTION, RETURNMESSAGE, State, Direction, IsServer, OriginalSenderId, SenderId, UserId, FinalReceiverId, ReceiverId, RemoteAddr, RemoteSAP, ProtocolFileLabel, Site, Filename, FileSize FROM ${input.view} LEFT JOIN subquery ON ${input.view}.coreid = subquery.SUBCOREID WHERE state IN( 'AVAILABLE', 'FAILED', 'SENT', 'RECEIVED' ) AND eventtimestamp IS NOT NULL AND COREID IN (SELECT SUBCOREID FROM subquery)`
    //search
    if(input.search){
        // let fieldsToSearch = ['cycleid','coreid','ProtocolFileName','UserId','Protocol','OriginalSenderId','RemoteAddr'];
        // let searchQuery = fieldsToSearch.join(` like '%${input.search}%' or `)
        // searchQuery = searchQuery + ` like '${input.search}'`
        query += ` AND (CONTAINS(STATE, '%${input.search}%', 1) > 0) `;
    }
   //transferDuration
  ...
  module.exports = formDownloadFileOracle;