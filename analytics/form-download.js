...
if (isSfrAccount && input?.filter?.fileName) {
...
if (isSfrAccount && input?.filter?.fileIdInfo) {
  ...
  if(input.search){
      ...
      if(input.search.includes('-')) {
        query += ` AND (CONTAINS(STATE, '${escaped_search_text}', 1) > 0) `;
      }
...
let input = {"libraryScriptId":"66829c836afa0b7484541185","view":"xfbtransfer_h","location":"34235236163","limit":"100","offset":"","search":"'testing1'.txt","transferDuration":"Recent","from":null,"to":null,"orderBy":null,"order":null,"filter":{"partnerName":null,"client":null,"loginName":null},"isLegacy":false,"partnerAccounts":null,"application":"NYL-AdvancedRoute"}


console.log(formDownloadFileOracle(input));