...
//when partnerAccount is 'all' then we need to filter on all the partner accounts
if (input?.filter?.partnerAccount && input?.filter?.partnerAccount.toLowerCase() === "all" && input?.partnerAccounts?.length > 0) {
  ...
  subQuery += ` AND (SenderId IN (${accountsToString}) OR OriginalSenderId IN (${accountsToString}) OR ReceiverId IN (${accountsToString}) OR FinalReceiverId IN (${accountsToString}))`;
}
//not needed for legacyAccount
if (isSfrAccount && input.businessUnit && input?.businessUnit.toLowerCase() != "all" && input?.businessUnit.toLowerCase() != "na") {
...
if (isSfrAccount && input.businessTag) {
    subQuery += ` AND UPPER(BusinessTag) = UPPER('${input.businessTag}')`;
}
