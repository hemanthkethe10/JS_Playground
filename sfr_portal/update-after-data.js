    ...
    try {
      ...
      if (input.pageData.currentRoutes.length) {
        input.pageData.currentRoutes.forEach((route) => {
          ...
          route.showRenameExample = input.pageData.isOutbound && route.renameFile.enabled === "true";
          ...
          route.destinations.forEach((destination) => {
            if (route.destinations.length) {
              ...
              //Encoding conversion
              fieldParam =
                input.pageData.currentRoutes[0].destinations[0].encodingConversions?.enabled;
              if (fieldParam) {
              ...
              if (input.pageData.isInbound) {
                ...
                route.requireSubscription = input.pageData.isInbound && input.pageData.route.source.name == "NYLhosted";
                route.requireSite = input.pageData.isInbound && input.pageData.route.source.name == "Partnerhosted";
                ...
                destination.destinationType === "InternalSite" ? (destination.destinationSites = input.pageData?.acountFolders?.filter(
                        (info) =>
                ...
                route.requireSubscription = input.pageData.isOutbound && destination.destinationType == "InternalSite";
                destination.requireSubscription = input.pageData.isInbound && destination.destinationType == "NYLhosted";
                destination.requireSite = input.pageData.isOutbound && destination.destinationType == "Partnerhosted";
              ...
              destination.selectedSite.server = selectedSite && destination.selectedSite.host;
                  //destination conditions for Inbound/Internal
              destination.isOutbound = JSON.stringify(input.pageData.sources) === JSON.stringify(input.pageData.destinations) ? !input.pageData.isInbound : input.pageData.isInbound;
                  //destination conditions for Outbound/Internal
               destination.isInbound = JSON.stringify(input.pageData.sources) === JSON.stringify(input.pageData.destinations) ? !input.pageData.isOutbound : input.pageData.isOutbound;
            ...
            //   destination.isOutbound = !input.pageData.isOutbound;
              ...
              destination.accountInfo = destination.accountName &&
              ...
              destination.showNewFolderAppAccount = destination.folderType === "newFolder" && destination.destinationType === "ApplicationAccount";
              destination.showExistingFolderAppAccount = destination.folderType === "existingFolder" && destination.destinationType === "ApplicationAccount";
