function run (input) {
  ...
  input.pageData.showClient = true;

  input.pageData.showPartnerAccountSource = input.pageData.route.direction === "Inbound";//update routing request view
...
if(input.pageData.route.requestType!=="New"){
 ...
 //For BU view page grey containers
  ...
  input.pageData.partnerAccounts.forEach((account)=>{
   ...
   let destinationAndSourcesMap={"sources":fileRouteCombinations.internalSources,"destinations":fileRouteCombinations.externalSources}
    ...
    if(input.pageData.route.currentRoutes.length){
     ...
     if (input.pageData.route.source?.folderType === "existingFolder") {
      ...
      route.itFolders = sourceFolders?.at(0)?.result;
       ...
       input.pageData.useExistingFileID = input.pageData.route.useExistingFileID === "true";
       if (input.pageData.useExistingFileID){
        route.showFileIdDetails = true;
        let fileInfoAfterSplit = route.fileId.split('-');
        let fileId = (fileInfoAfterSplit.length >= 2) ? fileInfoAfterSplit[0]: route.fileId;
        let classifier = (fileInfoAfterSplit.length >= 2) ? fileInfoAfterSplit[1]: route.fileId;
        let fileIDInfo = (fileId && classifier) ? input.pageData.fileIds.filter((it)=> it.fileId === fileId && it.fileDetailIndex === classifier) : input.pageData.fileIds.filter((it)=> it.fileId === route.fileId);
