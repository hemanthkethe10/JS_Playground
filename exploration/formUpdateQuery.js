function convertObjecttoPostgresUpdateQuery(input) {
    try{
    let data = input.data;
    ...
    let query = `UPDATE ${tableName} SET ${objectKeys.map((key, index) => `${key} = ${fomattedValues[index]}`).join(', ')}`;
    if (input.filter){
...
module.exports = convertObjecttoPostgresUpdateQuery;